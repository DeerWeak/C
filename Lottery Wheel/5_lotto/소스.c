...
int main()
{
	...
	else //파일이 열렸으면
	{
		...
		int* myNumber; //내 로또번호 저장하는 정수형 포인터 배열
		myNumber = (int*)malloc(6 * sizeof(int)); //int형 6개 만큼 동적할당 해준다.
		...
		while (1) { //계속 반복
			for (i = 0; i < 6; i++) { //6번 반복한다
				...
				for (i = 0; i < 6; i++) { //6번 반복한다
					for(j = 0; j < 6; j++) {
						if (myNumber[i] == lotto[j]) { //내 로또 번호가 뽑힌 로또 번호와 같은 것이 있다면
							hit++;
							}
						}
					}
